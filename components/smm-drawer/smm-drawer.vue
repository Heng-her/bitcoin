<template>
	<view :class="{drawerScope:true,hidden:openType}" ref="myDrawer" @touchstart="touchstartFn" @touchend="touchendFn"
		...
		<view :class="{drawer:true,open:openType}" :style="{width:width+'rpx'}" @touchstart="UseTouchstartTocloseBarFn"
			@touchend="UseTouchendTocloseBarFn">
...
*/
	...
	export default {
		...
		methods: {
			...
			closeDrawerFn(e) {
				// console.log(e.target===e.changedTouches);
				if (this.openType) {
					console.log('关闭界面');
					this.openType = false;
				}
